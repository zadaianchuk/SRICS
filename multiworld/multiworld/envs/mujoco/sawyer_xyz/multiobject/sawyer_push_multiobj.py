...
class SawyerMultiobjectPushEnv(MujocoEnv, Serializable, MultitaskEnv):
    ...
    def __init__(self,
        ...
        if self.mujoco_substeps:
            # We are using mujoco_py substeps instead of the one in MujocoEnv
            self.sim.nsubsteps = self.frame_skip

        if self.detect_contact:
            self.detected_contacts = None
            # we need one sub step to properly detect contacts 
            assert not self.mujoco_substeps
    ...
    def update_contacts(self, all_objects_ids):
        for i in range(self.sim.data.ncon):
            for k, id_1 in enumerate(all_objects_ids):
                for l, id_2 in enumerate(all_objects_ids):
                    if ((self.sim.data.contact[i].geom1 in id_1) and (self.sim.data.contact[i].geom2 in id_2)) or ((self.sim.data.contact[i].geom1 in id_2) and (self.sim.data.contact[i].geom2 in id_1)):
                        self.detected_contacts[k, l] = 1
    ...
    def step(self, action):
        ...
        if self.detect_contact:
            all_objects_ids = [[self.model.geom_name2id('leftclaw_it'), self.model.geom_name2id('rightclaw_it')]] + [[idx] for idx in self.mujoco_obj_geom_ids]
            n_all_objects = len(all_objects_ids)
            self.detected_contacts = np.zeros((n_all_objects, n_all_objects))
        for _ in range(self.action_repeat):
            self._set_action(action)
            for _ in range(substeps):   
                    self.sim.step()
                    if self.detect_contact:
                        self.update_contacts(all_objects_ids)

        self._set_stable_objects()
    ...
    def _ensure_objects_within_bounds(self):
        ...
        for qpos_adr in self.mujoco_obj_qpos_adr[:self.n_objects]:
            ...
            if np.any(qpos != clipped_qpos):
                ...
                self.sim.forward()
    def _set_stable_objects(self):
        """Reset coords of stable objects"""
        for qpos_adr, pos in zip([self.mujoco_obj_qpos_adr[i] for i in self.stable_objects], self.stable_objects_pos):
            self.data.qpos[qpos_adr:qpos_adr + 2] = pos
            self.sim.forward()
    
    # def _update_depended_objects(self):
    #     """Reset coords of stable objects"""
    #     for qpos_adr_i, qpos_adr_j in [self.mujoco_obj_qpos_adr[i], self.mujoco_obj_qpos_adr[j] for i, j in self.depended_objects]:
            
    #         self.data.qpos[qpos_adr:qpos_adr + 2] = delta
    #         self.sim.forward()
    ...
    def _get_obs(self):
        ...
        if self.detect_contact:
            detected_contacts = self.detected_contacts 
        else: 
    ...
    def _set_initial_object_positions(self):
        if self.randomize_object_init:
            ...
            self.stable_objects_pos = obj_pos[self.stable_objects]
        else:
    ...
    def _sample_goals(self):
        ...
        while True:
            ...
            delta = np.array([np.sin(phi), np.cos(phi)]) * self.spring_size
            for c in self.connections:
                obj_goals[c[1]] = obj_goals[c[0]] + delta
...
def _create_object_xml(output_dir, num_objects, friction_params,
                       cylinder_radius, connections, spring_size):
