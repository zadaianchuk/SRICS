...
def create_object_xml(filename, num_objects, object_mass, friction_params, object_meshes,
                      ...
                      use_textures = False, sliding_joints = False, connections = (), spring_size = 0.1):
    """
    :param hyperparams:
        ...
        if object_meshes is not None:
            ...
            if use_textures:
                ...
                ET.SubElement(obj, "geom", name=obj_string, pos="0 0 0", type="cylinder", size=str(cylinder_radius) + " 0.015",
                                        rgba="1 1 1 1", contype="182", conaffinity="20", material="mat_" + obj_string)

            else:
                ET.SubElement(obj, "geom", name=obj_string, pos="0 0 0", type="cylinder", size=str(cylinder_radius) + " 0.015",
                                        rgba="{} {} {} 1".format(color2[0], color2[1], color2[2]),
                                        contype="18", conaffinity="20", )
                ET.SubElement(obj, "site", name=f"site_{i}", pos="0 0 0", type="sphere", size=" 0.01")
    ...
    tendom = ET.SubElement(root, "tendon")
    for obj1, obj2 in connections:
        spatial = ET.SubElement(tendom, "spatial", limited="true",  range=f"0 {spring_size}",  width="0.005")
        ET.SubElement(spatial, "site", site=f"site_{obj1}")
        ET.SubElement(spatial, "site", site=f"site_{obj2}")
