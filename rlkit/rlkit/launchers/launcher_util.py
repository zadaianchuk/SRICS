...
def apply_hp_options(variant):
    ...
    def set_and_log2(dict_key1, dict_key2, key, value):
        logger.log('Setting {}.{}.{}={}'.format(dict_key1, dict_key2,
                                                key, value))
        d = variant.setdefault(dict_key1, {})
        d.setdefault(dict_key2, {})[key] = value
    ...
    if 'num_attention_heads' in options:
        num_heads = options['num_attention_heads']
        set_and_log2('qf_kwargs', 'attention_kwargs', 'num_heads', num_heads)
        set_and_log2('policy_kwargs', 'attention_kwargs', 'num_heads', num_heads)

    if 'attention_output_aggregation' in options:
        output_aggregation = options['attention_output_aggregation']
        set_and_log2('qf_kwargs', 'attention_kwargs', 'output_aggregation', output_aggregation)
        set_and_log2('policy_kwargs', 'attention_kwargs', 'output_aggregation', output_aggregation)
...
def run_experiment(
    ...
    if mode == 'here_no_doodad':
        ...
        if "log_dir" in variant:
            run_experiment_kwargs["log_dir"] = variant["log_dir"]
