...
class ObsDictRelabelingMultiObjectBuffer(ObsDictRelabelingBuffer):
    ...
    def random_batch(self, batch_size):
        ...
        if num_future_goals > 0:
            ...
            future_goal_goals = self._next_obs[self.desired_goal_key][future_obs_idxs]

            if "obs" in self.achieved_goal_key:
                object_indices = self._future_obs_idx_to_object_idx[future_obs_idxs]
                future_goals = self.env.extract_achieved_goals(future_goals, future_goal_goals, 
                                                               object_indices)
    ...
    def get_state(self):
        ...
        state["future_obs_idx_to_object_idx"] = self._future_obs_idx_to_object_idx
        return state

    def set_state(self, data):
        super().set_state(data)
        self._future_obs_idx_to_object_idx = data["future_obs_idx_to_object_idx"]
