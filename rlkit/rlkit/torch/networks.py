...
class SetGoalAttentionMlp(nn.Module):
    def __init__(self, 
                 embed_dim, 
                 goal_dims,
                 z_goal_size, 
        ...
        self.max_n_objects = max_objects 
        self.z_size = z_size
        self.n_frames = n_frames
        ...
        self.attention = SetGoalAttention(embed_dim,
                                   z_goal_size,
                                   z_size,
                                   n_queries=self.n_objects,
                                   **attention_kwargs)
        inp_dim = (self.attention.output_dim + self.attention.embed_dim + action_size)
        self.full_z_goal_size = self.n_objects * 2 + (z_goal_size - 1) * self.n_objects
        self.mlp = FlattenMlp(hidden_sizes, output_size, inp_dim, **kwargs)
    ...
    def forward(self, obs, actions):
        x, g, n_objects = preprocess_set_goal_attention_input(obs,
                                                     ...
                                                     self.n_frames)
        g, _, mask_subgoal = reshape_subgoals(g, self.goal_dims, self.n_objects, self.max_n_objects, self.z_goal_size, self.z_size)
        ...
        output = self.mlp(h, goal_embedding, actions)
